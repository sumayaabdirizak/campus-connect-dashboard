...
router.post('/:courseOfferingId', requireCourseOfferingRead(), asyncHandler(async (req, res) => {
  const courseOfferingId = req.courseOffering.id;
...
router.patch('/post/:postId', requireCoursePostRead(), asyncHandler(async (req, res) => {
...
router.delete('/post/:postId', requireCoursePostRead(), asyncHandler(async (req, res) => {
  const postId = parseInt(req.params.postId, 10);
  if (!Number.isInteger(postId)) {
    return res.status(400).json({ message: 'Invalid postId' });
  }
...
router.post(
  '/post/:postId/attachments', requireCoursePostRead(),
...
router.delete('/attachments/:attachmentId', requireCoursePostRead(), asyncHandler(async (req, res) => {
...
router.post('/post/:postId/reactions', requireCoursePostRead(), asyncHandler(async (req, res) => {
  const postId = parseInt(req.params.postId, 10);
...
router.post('/post/:postId/replies', requireCoursePostRead(), asyncHandler(async (req, res) => {
  const postId = parseInt(req.params.postId, 10);
...
router.patch('/replies/:replyId', requireCoursePostRead(), asyncHandler(async (req, res) => {
...
router.delete('/replies/:replyId', requireCoursePostRead(), asyncHandler(async (req, res) => {
  const replyId = parseInt(req.params.replyId, 10);
  const existing = await prisma.coursePostReply.findUnique({ where: { id: replyId } });
  if (!existing) return res.status(404).json({ message: 'Reply not found' });
  if (existing.authorId !== req.user.id) {

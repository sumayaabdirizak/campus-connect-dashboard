...
router.post('/:courseOfferingId/messages', requireCourseOfferingRead(), asyncHandler(async (req, res) => {
  const courseOfferingId = req.courseOffering.id;
...
router.patch('/messages/:messageId', requireChatMessageRead(), asyncHandler(async (req, res) => {
...
router.delete('/messages/:messageId', requireChatMessageRead(), asyncHandler(async (req, res) => {
  const messageId = parseInt(req.params.messageId, 10);
  const existing = await prisma.chatMessage.findUnique({
    where: { id: messageId },
...
router.post(
  '/messages/:messageId/attachments', requireChatMessageRead(),

...
/** Load a thread and assert the caller may see it. Returns { thread, isStaff }. */
export async function loadThreadScoped(threadId, userId) {
  ...
  const staff = await staffMembership(userId, thread.officeId);
  if (thread.studentId !== userId && !staff) {
    // 404 (not 403) so outsiders can't probe which thread ids exist.
    throw new HttpError(404, 'Conversation not found');
  }
  return { thread, isStaff: Boolean(staff) };
}

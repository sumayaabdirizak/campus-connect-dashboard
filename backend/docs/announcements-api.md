...
### Engagement (API fields)
Each `Announcement` in list/detail JSON includes:
- `likes`: number — count of heart reactions (`AnnouncementReaction` with emoji `❤️`)
- `commentsCount`: number — top-level comments where `deletedAt` is null
- `likedByCurrentUser`: boolean — whether the caller has the heart reaction on this announcement

### `POST /:id/like`
Toggle the caller’s heart reaction on a visible announcement. Response:
```json
{ "likes": 12, "likedByCurrentUser": true }
```

Comment threads are no longer exposed over HTTP on this router (use other channels if needed).

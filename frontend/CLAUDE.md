...
- **[docs/nav-rbac.md](./docs/nav-rbac.md)** — Navigation RBAC: role-based sidebar filtering
- **[docs/auth.md](./docs/auth.md)** — Cookie JWT auth, CSRF, session, environment variables
...
- **Icons** — for sidebar / navigation / app chrome, import from `@/components/icons` (Tabler icons via the curated barrel). For feature components, `lucide-react` is fine — it's the icon library shadcn ships with and is used throughout the feature folders. Never import from `@tabler/icons-react` directly.

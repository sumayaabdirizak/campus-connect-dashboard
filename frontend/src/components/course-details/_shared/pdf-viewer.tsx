...
import { ChevronLeft, ChevronRight, ScrollText, ZoomIn, ZoomOut } from 'lucide-react';
...
export function PdfViewer({ url, fillHeight }: PdfViewerProps) {
  const [numPages, setNumPages] = useState<number | null>(null);
  const [pageNumber, setPageNumber] = useState(1);
  const [scale, setScale] = useState(1);
  const [error, setError] = useState<string | null>(null);
  // Continuous scroll is the natural way to read a long document, so the
  // full-screen reader opens in it. The embedded viewer (grading drawer) sits
  // in a short box where paging still makes more sense.
  const [scrollMode, setScrollMode] = useState(!!fillHeight);
      ...
      <div className='flex items-center justify-between gap-2 flex-wrap'>
        <div className='flex items-center gap-1'>
          ...
          <Button
            ...
            title={scrollMode ? 'Showing all pages — click to view one at a time' : 'Scroll through all pages'}
          >

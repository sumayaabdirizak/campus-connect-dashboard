...
import { useLeaveGroupDm, useMarkNotificationsRead } from '@/lib/discussions/queries/queries'
import { emitMessageRead } from '@/lib/discussions/queries/socket'
...
export function useDmPaneEffects({
  ...
  const leaveMut = useLeaveGroupDm(groupDmId ?? '')
  const markRead = useMarkNotificationsRead()

  const lastEmittedReadIdRef = useRef<string | null>(null)
  useEffect(() => {

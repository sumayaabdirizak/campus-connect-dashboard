...
import { ensureSocket } from './socket-connection';
import { getSocketRef, roomRefCounts, type RoomKey } from './socket-state';
...
function emitLeaveForRoom(s: Socket, room: RoomKey) {
  if (room.startsWith('channel:')) {
    s.emit('channel:leave', { channelId: Number(room.split(':')[1]) });
  } else if (room.startsWith('groupdm:')) {
    s.emit('groupdm:leave', { groupDmId: Number(room.split(':')[1]) });
  } else if (room.startsWith('discussion:')) {
    s.emit('leave:group', { groupId: Number(room.split(':')[1]) });
  }
}

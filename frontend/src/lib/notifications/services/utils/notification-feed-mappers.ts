...
import type { DiscussionNotification } from '@/lib/notifications/types';
...
export function discHref(n: DiscussionNotification): string {
  if (String(n.type).startsWith('CLUB')) return '/dashboard/messages?discover=1';
  const p = n.payload ?? {};
  const g = p.groupId ?? n.groupId ?? undefined;
  const c = p.channelId;
  if (g && c) return `/dashboard/messages?server=${g}&channel=${c}`;
  if (g) return `/dashboard/messages?server=${g}`;

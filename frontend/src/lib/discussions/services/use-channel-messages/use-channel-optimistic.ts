...
import type { DiscussionMessage, MessageReaction } from '@/lib/discussions/queries/types'
...
export function useChannelOptimistic(
  ...
  const replaceOptimisticMessage = useCallback(
    (tempId: string, real: DiscussionMessage) => {
      setState((s) => {
  ...
  const optimisticPatchMessage = useCallback(
    (messageId: string, patch: Partial<DiscussionMessage>): (() => void) => {

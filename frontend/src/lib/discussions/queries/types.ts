...
export type DiscussionChannel = {
  ...
  /**
   * Inbox-matching title for legacy dept/batch/section channels
   * (e.g. "BSC-MATH-B1"). Common channels leave this null.
   */
...
export type DiscussionAuditLogEntry = {
  id: number;
  /** Left as raw internal integer id — audit log is an admin-only view, not part of the UUID migration. */
  serverId: number;
...
export type GroupDmCandidatesResponse = {
  ...
  /** Desk / course reach for New message copy. */
  dmScope?: 'university' | 'faculty' | 'none' | 'courses';
};
...
/** Top-bit "skip-all-checks" flag. Server owner / SUPER_ADMIN / faculty discussion admins. */

...
Configure in `src/config/nav-config.ts`. Items are filtered client-side in `src/hooks/use-nav.ts` by `user.role` from `useAuthStore`.

**Access control properties** on nav items (Campus Connect uses `roles`):

- `roles: Role[]` — show only for these campus roles (e.g. `STUDENT`, `TEACHER`, `DEAN`)
